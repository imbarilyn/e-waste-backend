...
def get_user_by_email(db: pymysql.connections.Connection, email: str):
    with db.cursor() as cursor:
        query = """
        SELECT * FROM e_waste.admin WHERE email = %s
        """
...
def get_user(phone_number: str, email: str, db: pymysql.connections.Connection):
    with db.cursor() as cursor:
        query = "SELECT * FROM e_waste.admin WHERE phone_number = %s OR  email = %s"
...
def authenticate_user(phone_number: str, password: str, db: pymysql.connections.Connection):
    ...
    with db.cursor() as cursor:
        query = """
        SELECT * FROM e_waste.admin WHERE phone_number = %s
        """
...
def create_reset_token(admin_id: str , db: pymysql.connections.Connection):
    EXPIRY_DURATION = 24
    ...
    with db.cursor() as cursor:
        try:
            ...
            VALUES (%s, %s, %s, %s, %s)
            """
            cursor.execute(query, (reset_password_id, admin_id, reset_token, reset_token_expiry, 'admin'))
            db.commit()
            select_query = """
    ...
    if everything_valid:
        ...
        with db.cursor() as cursor:
            try:
                ...
                WHERE id = %s
                """
                cursor.execute(query_admin, (hashed_new_password, everything_valid['user_id']))
